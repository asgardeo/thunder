...
// TestAcrClaimAbsentWithoutAcrValues verifies that when no acr_values are requested
// and no ACR is selected, the ID token does not contain an acr claim.
func (ts *AcrIDTokenTestSuite) TestAcrClaimAbsentWithoutAcrValues() {
	// Step 1: Initiate authorization without acr_values.
	...
	// Step 2: Resume flow — both ACR options shown (no filtering).
	flowStep, err := testutils.ExecuteAuthenticationFlow(flowID, nil, "")
	ts.Require().NoError(err)
	ts.Require().Equal("INCOMPLETE", flowStep.FlowStatus)
	...
	// Step 5: Decode ID token — acr claim should still be present since the login_options
	// node records the selected ACR even when no acr_values filter was applied.
	idTokenClaims, err := testutils.DecodeJWT(tokenResult.Token.IDToken)
	ts.Require().NoError(err)

	acrClaim, ok := idTokenClaims.Additional["acr"]
	ts.Assert().True(ok, "acr claim should be present when an ACR option was selected")
	if ok {
		ts.Assert().Equal("mosip:idp:acr:password", acrClaim,
