...
func (suite *ActionAPITestSuite) TestCreateActionAtResource() {
	...
	suite.Equal(suite.resourceID, *action.ResourceID)
}
...
func (suite *ActionAPITestSuite) TestCreateActionSameHandleDifferentLevels() {
	...
	// Verify both exist
	serverAction, err := getActionAtResourceServer(suite.resourceServerID, serverActionID)
	suite.Require().NoError(err)
	suite.Nil(serverAction.ResourceID)

	resourceAction, err := getActionAtResource(suite.resourceServerID, suite.resourceID, resourceActionID)
	suite.Require().NoError(err)
	suite.NotNil(resourceAction.ResourceID)
	suite.Equal(suite.resourceID, *resourceAction.ResourceID)
}
...
func (suite *ActionAPITestSuite) TestListActionsAtResource() {
	...
	foundAction2 := false
	for _, action := range list.Actions {
		suite.NotNil(action.ResourceID, "All actions should be at resource level")
		suite.Equal(suite.resourceID, *action.ResourceID)
