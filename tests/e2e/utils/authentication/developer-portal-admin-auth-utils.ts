...
export function loadAuthState(filePath: string, debug: boolean = false): AuthState {
  if (!fs.existsSync(filePath)) {
    throw new Error(
      `Authentication state file not found: ${filePath}`
    );
  }
 ...
 */
export async function restoreCookies(context: BrowserContext, authState: AuthState, debug: boolean = false): Promise<void> {
...
export async function verifyAuthState(page: Page, debug: boolean = false): Promise<boolean> {
  ...
  const hasSessionData = Object.keys(sessionStorage).some(key =>
    key.includes('session_data-instance_0')
  );
  ...
  if (debug) {
    ...
    console.log(`🔍 [DEBUG] Session active: ${isSessionActive}, Has session data: ${hasSessionData}, Tokens valid: ${tokensValid}`);
  }

  console.log(`🔍 Auth verification: session active: ${isSessionActive}, has session data: ${hasSessionData}, tokens valid: ${tokensValid}`);
...
export async function setupAuthentication(
  ...
  console.log(`Loaded auth state: ${authState.origins?.[0]?.localStorage?.length || 0} localStorage, ${authState.origins?.[0]?.sessionStorage?.length || 0} sessionStorage items`);
...
function checkTokensExpired(authState: AuthState, debug: boolean): boolean {
  const sessionDataKey = authState.origins?.[0]?.sessionStorage?.find(
    item => item.name.includes('session_data-instance_0')
  );
...
async function performInlineLogin(
  ...
  if (!username || !password) {
    throw new Error('ADMIN_USERNAME and ADMIN_PASSWORD environment variables are required for inline login');
  }
  ...
  // Click sign in
  const signInButton = page.locator('button[type="submit"]').or(
    page.getByRole('button', { name: /sign in|login|submit/i })
  );
  await signInButton.first().click();

  // Wait for redirect to develop page
  await page.waitForURL('**/develop/**', { timeout: 20000 });
...
async function saveAuthState(
  ...
  if (debug) {
    console.log(`🔍 [DEBUG] Saved: ${cookies.length} cookies, ${localStorage.length} localStorage, ${sessionStorage.length} sessionStorage`);
  }
}
