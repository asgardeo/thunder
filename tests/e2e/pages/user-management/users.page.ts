...
export class UsersPage {
    ...
    constructor(page: Page, baseUrl: string) {
        ...
        this.firstNameInput = page.locator('input[name="given_name"], input[name="firstName"]')
            .or(page.getByLabel(/first.*name|given.*name/i));

        this.lastNameInput = page.locator('input[name="family_name"], input[name="lastName"]')
        ...
        // Form heading
        this.formHeading = page.locator('h1, h2, h3, h4, h5, h6')
    ...
    async isOnUsersPage(): Promise<boolean> {
        ...
        return url.includes(DeveloperPortalRoutes.users) &&
            !url.includes(DeveloperPortalRoutes.signin);
    }
    ...
    async clickAddUser() {
        await this.addUserButton.first().waitFor({ state: 'visible', timeout: 5000 });
    ...
    async waitForUserForm() {
        await expect(this.formHeading.first()).toBeVisible({ timeout: 10000 });
    }
    ...
    async screenshot(name: string) {
        const screenshotPath = path.join('test-results', 'debug', `${name}.png`);
