...
export class DeveloperPortalSigninPage {
    readonly page: Page;
    ...
    async isOnLoginPage(): Promise<boolean> {
        ...
        return url.includes(DeveloperPortalRoutes.signin) ||
            url.includes('/auth') ||
            url.includes('/login');
    }
    ...
    async waitForLoginForm() {
        await this.usernameInput.first().waitFor({ state: 'visible', timeout: 10000 });
    }
    ...
    async waitForLoginSuccess() {
        await this.page.waitForURL(`**${DeveloperPortalRoutes.home}/**`, { timeout: 20000 });
