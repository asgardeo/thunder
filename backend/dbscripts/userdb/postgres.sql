...
CREATE TABLE "ORGANIZATION_UNIT" (
    ...
    METADATA         JSONB CHECK (METADATA IS NULL OR octet_length(METADATA::text) <= 65536),
...
CREATE TABLE "ENTITY" (
    ...
    ATTRIBUTES          JSONB CHECK (ATTRIBUTES IS NULL OR octet_length(ATTRIBUTES::text) <= 65536),
    SYSTEM_ATTRIBUTES   JSONB CHECK (SYSTEM_ATTRIBUTES IS NULL OR octet_length(SYSTEM_ATTRIBUTES::text) <= 65536),
    CREDENTIALS         JSONB CHECK (CREDENTIALS IS NULL OR octet_length(CREDENTIALS::text) <= 16384),
    SYSTEM_CREDENTIALS  JSONB CHECK (SYSTEM_CREDENTIALS IS NULL OR octet_length(SYSTEM_CREDENTIALS::text) <= 16384),
    CREATED_AT          TIMESTAMPTZ DEFAULT NOW(),
    UPDATED_AT          TIMESTAMPTZ DEFAULT NOW()
);
