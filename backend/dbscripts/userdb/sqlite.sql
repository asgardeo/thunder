...
CREATE TABLE IF NOT EXISTS GROUP_MEMBER_REFERENCE (
    ...
    FOREIGN KEY (GROUP_ID, DEPLOYMENT_ID) REFERENCES "GROUP" (ID, DEPLOYMENT_ID) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS USER_INDEXED_ATTRIBUTES (
    ...
    FOREIGN KEY (USER_ID, DEPLOYMENT_ID) REFERENCES USER (ID, DEPLOYMENT_ID) ON DELETE CASCADE
);
