...
CREATE TABLE "ORGANIZATION_UNIT" (
    ...
    METADATA     TEXT CHECK (METADATA IS NULL OR length(METADATA) <= 65536),
...
CREATE TABLE "ENTITY" (
    ...
    ATTRIBUTES          TEXT CHECK (ATTRIBUTES IS NULL OR length(ATTRIBUTES) <= 65536),
    SYSTEM_ATTRIBUTES   TEXT CHECK (SYSTEM_ATTRIBUTES IS NULL OR length(SYSTEM_ATTRIBUTES) <= 65536),
    CREDENTIALS         TEXT CHECK (CREDENTIALS IS NULL OR length(CREDENTIALS) <= 16384),
    SYSTEM_CREDENTIALS  TEXT CHECK (SYSTEM_CREDENTIALS IS NULL OR length(SYSTEM_CREDENTIALS) <= 16384),
    CREATED_AT          TEXT DEFAULT (datetime('now')),
    UPDATED_AT          TEXT DEFAULT (datetime('now'))
);
