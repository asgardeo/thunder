...
CREATE TABLE "ENTITY_TYPES" (
    ...
    SCHEMA_DEF  TEXT NOT NULL CHECK (length(SCHEMA_DEF) <= 65536),
    SYSTEM_ATTRIBUTES TEXT CHECK (SYSTEM_ATTRIBUTES IS NULL OR length(SYSTEM_ATTRIBUTES) <= 65536),
...
CREATE TABLE "THEME" (
    ...
    THEME TEXT NOT NULL CHECK (length(THEME) <= 1048576),
    CREATED_AT TEXT DEFAULT (datetime('now')),
    UPDATED_AT TEXT DEFAULT (datetime('now')),
...
CREATE TABLE "LAYOUT" (
    ...
    HANDLE VARCHAR(255) NOT NULL,
    DESCRIPTION VARCHAR(512),
    LAYOUT TEXT NOT NULL CHECK (length(LAYOUT) <= 1048576),
    CREATED_AT TEXT DEFAULT (datetime('now')),
    UPDATED_AT TEXT DEFAULT (datetime('now')),
    UNIQUE (DEPLOYMENT_ID, HANDLE)
);
...
CREATE TABLE "INBOUND_CLIENT" (
    ...
    PROPERTIES TEXT CHECK (PROPERTIES IS NULL OR length(PROPERTIES) <= 32768),
...
CREATE TABLE "OAUTH_INBOUND_PROFILE" (
    ...
    OAUTH_CONFIG TEXT CHECK (OAUTH_CONFIG IS NULL OR length(OAUTH_CONFIG) <= 32768),
...
CREATE TABLE "IDP" (
    ...
    PROPERTIES TEXT CHECK (PROPERTIES IS NULL OR length(PROPERTIES) <= 65536),
    CREATED_AT TEXT DEFAULT (datetime('now')),
    UPDATED_AT TEXT DEFAULT (datetime('now'))
);
...
CREATE TABLE "FLOW_VERSION" (
    ...
    NODES TEXT NOT NULL CHECK (length(NODES) <= 1048576),
    CREATED_AT TEXT DEFAULT (datetime('now')),
