...
var (
	...
	queryGetUserSchemaList = dbmodel.DBQuery{
		...
		Query: `SELECT SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, DISPLAY_ATTRIBUTE FROM USER_SCHEMAS ` +
	...
	queryCreateUserSchema = dbmodel.DBQuery{
		...
		Query: `INSERT INTO USER_SCHEMAS ` +
			`(SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, DISPLAY_ATTRIBUTE, SCHEMA_DEF, DEPLOYMENT_ID) ` +
	...
	queryGetUserSchemaByID = dbmodel.DBQuery{
		...
		Query: `SELECT SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, ` +
			`DISPLAY_ATTRIBUTE, SCHEMA_DEF FROM USER_SCHEMAS ` +
	...
	queryUpdateUserSchemaByID = dbmodel.DBQuery{
		...
		Query: `UPDATE USER_SCHEMAS
			SET NAME = $1, OU_ID = $2, ALLOW_SELF_REGISTRATION = $3, DISPLAY_ATTRIBUTE = $4, SCHEMA_DEF = $5
