...
var (
	...
	queryGetUserSchemaList = dbmodel.DBQuery{
		...
		Query: `SELECT SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SYSTEM_ATTRIBUTES FROM USER_SCHEMAS ` +
	...
	queryCreateUserSchema = dbmodel.DBQuery{
		...
		Query: `INSERT INTO USER_SCHEMAS (SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SYSTEM_ATTRIBUTES, SCHEMA_DEF, DEPLOYMENT_ID)
	...
	queryGetUserSchemaByID = dbmodel.DBQuery{
		...
		Query: `SELECT SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SYSTEM_ATTRIBUTES, SCHEMA_DEF FROM USER_SCHEMAS ` +
	...
	queryUpdateUserSchemaByID = dbmodel.DBQuery{
		...
		Query: `UPDATE USER_SCHEMAS
			SET NAME = $1, OU_ID = $2, ALLOW_SELF_REGISTRATION = $3, SYSTEM_ATTRIBUTES = $4, SCHEMA_DEF = $5
			WHERE SCHEMA_ID = $6 AND DEPLOYMENT_ID = $7`,
	}
...
func buildGetUserSchemaListByOUIDsQuery(ouIDs []string) dbmodel.DBQuery {
	...
	if n == 0 {
		return dbmodel.DBQuery{
			...
			PostgresQuery: `SELECT SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SYSTEM_ATTRIBUTES FROM USER_SCHEMAS ` +
				`WHERE 1=0 AND DEPLOYMENT_ID = $1 ORDER BY NAME LIMIT $2 OFFSET $3`,
			SQLiteQuery: `SELECT SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SYSTEM_ATTRIBUTES FROM USER_SCHEMAS ` +
