...
func TestBuildGetUserSchemaListByOUIDsQuery(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			name:  "Empty OUIDs",
			ouIDs: []string{},
			wantPG: `SELECT SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SYSTEM_ATTRIBUTES FROM USER_SCHEMAS ` +
				`WHERE 1=0 AND DEPLOYMENT_ID = $1 ORDER BY NAME LIMIT $2 OFFSET $3`,
			wantSQLite: `SELECT SCHEMA_ID, NAME, OU_ID, ALLOW_SELF_REGISTRATION, SYSTEM_ATTRIBUTES FROM USER_SCHEMAS ` +
