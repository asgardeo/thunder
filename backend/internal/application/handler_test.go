...
// mockApplicationService is a mock for ApplicationServiceInterface for handler testing
type mockApplicationService struct {
	mock.Mock
}

func (m *mockApplicationService) CreateApplication(
	...
	return args.Get(0).(*model.ApplicationDTO), nil
}

func (m *mockApplicationService) ValidateApplication(app *model.ApplicationDTO) (
	*model.ApplicationProcessedDTO, *model.InboundAuthConfigDTO, *serviceerror.ServiceError) {
	args := m.Called(app)
	if args.Get(0) == nil {
		return nil, nil, args.Get(2).(*serviceerror.ServiceError)
	}
	return args.Get(0).(*model.ApplicationProcessedDTO), args.Get(1).(*model.InboundAuthConfigDTO), nil
}

func (m *mockApplicationService) GetApplicationList() (*model.ApplicationListResponse, *serviceerror.ServiceError) {
	args := m.Called()
	if args.Get(0) == nil {
		return nil, args.Get(1).(*serviceerror.ServiceError)
	}
	return args.Get(0).(*model.ApplicationListResponse), nil
}

func (m *mockApplicationService) GetOAuthApplication(
	clientID string,
) (*model.OAuthAppConfigProcessedDTO, *serviceerror.ServiceError) {
	args := m.Called(clientID)
	if args.Get(0) == nil {
		return nil, args.Get(1).(*serviceerror.ServiceError)
	}
	return args.Get(0).(*model.OAuthAppConfigProcessedDTO), nil
}

func (m *mockApplicationService) GetApplication(
	appID string,
	...
	return args.Get(0).(*model.ApplicationProcessedDTO), nil
}

func (m *mockApplicationService) UpdateApplication(
	appID string, app *model.ApplicationDTO,
) (*model.ApplicationDTO, *serviceerror.ServiceError) {
...
func (suite *HandlerTestSuite) TestHandleApplicationGetRequest_Success() {
	...
	expectedApp := &model.ApplicationProcessedDTO{
		ID:          "test-app-id",
		Name:        "TestApp",
		Description: "Test Description",
	}
...
func (suite *HandlerTestSuite) TestHandleApplicationGetRequest_UnsupportedAuthTypeErrorEncodingFails() {
	...
	app := &model.ApplicationProcessedDTO{
		...
		InboundAuthConfig: []model.InboundAuthConfigProcessedDTO{
			{
				Type:           "UNSUPPORTED_TYPE", // Not OAuthInboundAuthType
				OAuthAppConfig: &model.OAuthAppConfigProcessedDTO{ClientID: "test-client-id"},
			},
		},
	}
