...
func (m *magicLinkAuthExecutor) InitiateMagicLink(ctx *core.NodeContext) (*common.ExecutorResponse, error) {
	...
	if failure := m.sendMagicLink(email, ctx, logger); failure != "" {
		execResp.Status = common.ExecFailure
...
func (m *magicLinkAuthExecutor) ProcessAuthFlowResponse(ctx *core.NodeContext) (*common.ExecutorResponse, error) {
	...
	userID, failure := m.validateMagicLinkToken(ctx, logger)
	if failure != "" {
		execResp.Status = common.ExecFailure
		execResp.FailureReason = failure
		return execResp, nil
...
func (m *magicLinkAuthExecutor) validateMagicLinkToken(ctx *core.NodeContext,
	...
	if !ok || token == "" {
		...
		return "", "Magic link token is required"
	}
	...
	if svcErr != nil {
		...
		return "", failureReasonInvalidMagicLinkToken
	}

	if user == nil {
		logger.Error("User not found after magic link verification")
		return "", failureReasonUserNotFound
	}
