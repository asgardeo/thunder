...
func (suite *BasicAuthExecutorTestSuite) TestValidateAMRForSelectedACR_ACRNotInMapping() {
	config.ResetThunderRuntime()
	_ = config.InitializeThunderRuntime("/tmp/test", &config.Config{
		ACRAMRMapping: config.ACRAMRMappingConfig{
			AMR:    map[string]config.AMRFactor{"pwd": {Type: "password"}},
			AcrAMR: map[string][]string{"loa1": {"pwd"}},
		},
	})
	defer func() {
		config.ResetThunderRuntime()
	...
	ctx := &core.NodeContext{
		RuntimeData: map[string]string{
			common.RuntimeKeySelectedAuthClass: "unknown_acr",
		},
	}
	assert.Empty(suite.T(), suite.executor.validateAMRForSelectedACR(ctx))
}
