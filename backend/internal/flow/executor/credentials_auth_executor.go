...
func (b *credentialsAuthExecutor) Execute(ctx *core.NodeContext) (*common.ExecutorResponse, error) {
	...
	logger.Debug("Executing credentials authentication executor")
	...
	if hasPreResolvedUser {
		...
		logger.Debug("Required inputs for credentials authentication executor is not provided")
		execResp.Status = common.ExecUserInputRequired
		return execResp, nil
	}
	...
	logger.Debug("Credentials authentication executor execution completed",
...
func (b *credentialsAuthExecutor) getAuthenticatedUser(ctx *core.NodeContext,
