...
var (
	...
	QueryCreateFlowUserData = model.DBQuery{
		...
		Query: "INSERT INTO FLOW_USER_DATA (FLOW_ID, IS_AUTHENTICATED, USER_ID, " +
			"OU_ID, USER_TYPE, USER_INPUTS, USER_ATTRIBUTES, FLOW_CONTEXT_ID, DEPLOYMENT_ID) " +
			"VALUES ($1, $2, $3, $4, $5, $6, $7, (SELECT ID FROM FLOW_CONTEXT WHERE FLOW_ID = $1 AND DEPLOYMENT_ID = $8), $8)",
	}
	...
	QueryGetFlowContextWithUserData = model.DBQuery{
		ID: "FLQ-FLOW_CTX-05",
		Query: `SELECT 
			fc.ID, fc.FLOW_ID, fc.APP_ID, fc."VERBOSE", fc.CURRENT_NODE_ID, fc.CURRENT_ACTION, 
		...
		LEFT JOIN FLOW_USER_DATA fud ON fc.ID = fud.FLOW_CONTEXT_ID AND fc.DEPLOYMENT_ID = $2 AND fud.DEPLOYMENT_ID = $2
