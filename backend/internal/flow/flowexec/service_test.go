	...
	mockCrypto.EXPECT().Encrypt(mock.Anything, mock.Anything, mock.Anything, mock.Anything).
		RunAndReturn(
			func(
				ctx context.Context,
				_ *kmprovider.KeyRef,
				_ crypto_lib.AlgorithmParams,
				content []byte) ([]byte, *crypto_lib.CryptoDetails, error) {
...
func TestEncryptDecryptRoundTrip_AllFieldsPreserved(t *testing.T) {
	...
	mockCrypto.EXPECT().Decrypt(mock.Anything, mock.Anything, mock.Anything, mock.Anything).
		RunAndReturn(func(
			ctx context.Context,
			_ *kmprovider.KeyRef,
			_ crypto_lib.AlgorithmParams, content []byte) ([]byte, error) {
	...
	decryptedBytes, err := mockCrypto.Decrypt(
		...
		crypto_lib.AlgorithmParams{Algorithm: crypto_lib.AlgorithmAESGCM},
