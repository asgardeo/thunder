...
func newRedisFlowStore(p provider.RedisClientInterface) flowStoreInterface {
...
func (s *redisFlowStore) StoreFlowContext(ctx context.Context, engineCtx EngineContext, expirySeconds int64) error {
	...
	dbModel.ExpiryTime = time.Now().UTC().Add(time.Duration(expirySeconds) * time.Second)
...
func (s *redisFlowStore) GetFlowContext(ctx context.Context, flowID string) (*FlowContextWithUserDataDB, error) {
