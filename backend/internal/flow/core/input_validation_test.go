...
func (s *InputValidationTestSuite) TestPromptNodeValidatesOnlySelectedActionInputs() {
	...
	// submit_phone with an all-digit value must pass; the previous dedup behavior would have applied Prompt A's '@' rule.
	ctx := &NodeContext{
...
func (s *InputValidationTestSuite) TestApplyValidationFailureRePrompt_PreservesStrippedDownInitialSet() {
	...
	s.Equal("password", nodeResp.Inputs[0].Identifier, "only the failing password (initially prompted) must be re-prompted")
