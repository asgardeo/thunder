...
const (
	...
	// defaultOriginHTTPAlt is the alternative default HTTP origin for local development.
	defaultOriginHTTPAlt = "http://localhost:8090"
	// assertionTTLMinutes is the TTL for assertion tokens in minutes.
	assertionTTLMinutes = 15
	// aal2Level represents Authentication Assurance Level 2 (multi-factor).
	aal2Level = "aal2"
)
...
func getConfiguredOrigins() []string {
	...
	defaultOrigins := []string{defaultOriginHTTP, defaultOriginHTTPAlt}
	...
	func() {
		...
		if runtime != nil {
			originList = runtime.Config.WebAuthn.AllowedOrigins
		}
	}()
...
// buildUserName builds a username from user attributes.
func buildUserName(userID string, attributes map[string]interface{}) string {
	if attributes == nil {
		return userID
	}
...
// extractUserInfo extracts display name and username from user info.
func extractUserInfo(userInfo *user.User) (displayName, userName string) {
	attributes := parseUserAttributes(userInfo.Attributes)
	displayName = buildUserDisplayName(userInfo.ID, attributes)
	userName = buildUserName(userInfo.ID, attributes)
	return displayName, userName
}

// buildAssertionClaims builds JWT assertion claims for authentication.
func buildAssertionClaims(userInfo *user.User, authenticator string) map[string]interface{} {
	now := time.Now()
	return map[string]interface{}{
		"sub":    userInfo.ID,
		"type":   userInfo.Type,
		"ou":     userInfo.OrganizationUnit,
		"amr":    []string{authenticator},
		"iat":    now.Unix(),
		"exp":    now.Add(assertionTTLMinutes * time.Minute).Unix(),
		"aal":    aal2Level,
		"authAt": now.Unix(),
	}
}
...
func parseAssertionResponse(credentialID, credentialType, clientDataJSON,
	authenticatorData, signature, userHandle string) (*protocol.ParsedCredentialAssertionData, error) {
...
func validateRegistrationStartRequest(req *WebAuthnRegisterStartRequest) *serviceerror.ServiceError {
...
// initializeWebAuthnLibrary initializes the WebAuthn library with configuration.
func initializeWebAuthnLibrary(relyingPartyID, rpDisplayName string) (*webauthn.WebAuthn, error) {
...
// initializeWebAuthnWithRP initializes the WebAuthn library with relying party configuration.
// This is a convenience method when display name equals the relying party ID.
func initializeWebAuthnWithRP(relyingPartyID string) (*webauthn.WebAuthn, error) {
	return initializeWebAuthnLibrary(relyingPartyID, relyingPartyID)
}

// buildRegistrationOptions builds registration options from the request.
func buildRegistrationOptions(req *WebAuthnRegisterStartRequest) []webauthn.RegistrationOption {
