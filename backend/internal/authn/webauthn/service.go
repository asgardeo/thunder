...
func (w *webAuthnAuthnService) StartRegistration(
	...
	webAuthnLib, err := initializeWebAuthnLibrary(req.RelyingPartyID, rpDisplayName)
	if err != nil {
		logger.Error("Failed to initialize WebAuthn library", log.String("error", err.Error()))
		return nil, &ErrorInternalServerError
	}
	...
	// Begin registration ceremony using the WebAuthn library
	options, sessionData, err := webAuthnLib.BeginRegistration(webAuthnUserAdapter, registrationOptions...)
	if err != nil {
...
func (w *webAuthnAuthnService) FinishRegistration(req *WebAuthnRegisterFinishRequest) (
	*WebAuthnRegisterFinishData, *serviceerror.ServiceError) {
	...
	// Initialize WebAuthn library with relying party configuration
	webAuthnLib, err := initializeWebAuthnWithRP(relyingPartyID)
	if err != nil {
		logger.Error("Failed to initialize WebAuthn library", log.String("error", err.Error()))
		return nil, &ErrorInternalServerError
	}

	// Verify the credential creation response using WebAuthn library
	// The ParsedCredentialCreationData has already been validated by the protocol package
	// Now we use the library to perform full WebAuthn verification
	logger.Debug("Calling CreateCredential with session data",
	...
	credential, err := webAuthnLib.CreateCredential(webAuthnUserAdapter, *sessionData, parsedCredential)
	if err != nil {
		logger.Error("Failed to verify and create credential", log.String("error", err.Error()))
		return nil, &ErrorInvalidAttestationResponse
	}

	// The library has now verified:
	...
	// - User verified (UV) flag if required
	// - Attestation statement is valid
	// - All cryptographic signatures are correct
...
func (w *webAuthnAuthnService) StartAuthentication(userID, relyingPartyID string) (
	*WebAuthnStartData, *serviceerror.ServiceError) {
	...
	// Begin login ceremony using the WebAuthn library
	// The library will generate challenge and set timeout automatically
	options, sessionData, err := webAuthnLib.BeginLogin(webAuthnUserAdapter)
	if err != nil {
...
func (w *webAuthnAuthnService) FinishAuthentication(credentialID, credentialType, clientDataJSON,
	...
	// Verify the credential assertion using WebAuthn library
	// This performs full WebAuthn verification:
	// - Challenge matches the stored challenge
	// - Origin matches the expected origin
	// - Type is "webauthn.get"
	// - RP ID hash in authenticatorData is correct
	// - User present (UP) flag is set
	// - User verified (UV) flag if required
	// - Signature is valid using the credential's public key
	// - Sign count is greater than stored count (prevents replay attacks)
	credential, err := webAuthnLib.ValidateLogin(webAuthnUserAdapter, *sessionData, parsedResponse)
	if err != nil {
	...
	// The credential object now has the updated sign count
...
// Note: In production, this should properly decode and re-sign JWT tokens
