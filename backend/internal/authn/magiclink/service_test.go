...
func (suite *MagicLinkServiceTestSuite) TestSendMagicLinkSuccess() {
	...
	suite.mockJWTService.On("GenerateJWT",
		...
		mock.MatchedBy(func(claims map[string]interface{}) bool {
			return len(claims) == 1 && claims[claimKeyRecipient] == testRecipient
		}),
		jwt.TokenTypeJWT,
	...
	suite.mockTemplateService.On("Render", mock.Anything, template.ScenarioMagicLink,
		mock.MatchedBy(func(data template.TemplateData) bool {
			...
			return strings.Contains(magicLinkValue, "flowId="+testFlowID) &&
				strings.Contains(magicLinkValue, "magicLinkToken="+testToken)
		})).
	...
	err := suite.service.SendMagicLink(context.Background(), testRecipient, int64(DefaultExpirySeconds), testFlowID, "")
	suite.Nil(err)
}
...
func (suite *MagicLinkServiceTestSuite) TestSendMagicLinkUserProviderError() {
	...
	err := svc.SendMagicLink(context.Background(), testRecipient, int64(DefaultExpirySeconds), testFlowID, "")
	suite.Nil(err)
}
...
func (suite *MagicLinkServiceTestSuite) TestSendMagicLinkEmailSendError() {
	...
	err := svc.SendMagicLink(context.Background(), testRecipient, int64(DefaultExpirySeconds), testFlowID, "")
	suite.NotNil(err)
	suite.Equal(ErrorEmailSendFailed.Code, err.Code)
}
...
func (suite *MagicLinkServiceTestSuite) TestVerifyMagicLinkSuccess() {
	...
	mockUserSvc.On("IdentifyUser", mock.MatchedBy(func(filters map[string]interface{}) bool {
		return filters["email"] == testRecipient
	})).Return(&testUserID, nil)
...
func (suite *MagicLinkServiceTestSuite) TestIsValidEmail() {
	...
	for _, tc := range tests {
		suite.Run(tc.email, func() {
			...
			suite.Equal(tc.expected, result, "isValidEmail(%q) should be %v", tc.email, tc.expected)
		})
	}
}

func (suite *MagicLinkServiceTestSuite) TestExtractString() {
	tests := []struct {
		name     string
		input    interface{}
		expected string
	...
	for _, tc := range tests {
		suite.Run(tc.name, func() {
			...
			suite.Equal(tc.expected, result, "extractString(%v) should be %q", tc.input, tc.expected)
		})
	}
}
...
func (suite *MagicLinkServiceTestSuite) TestSendMagicLinkTemplateRenderError() {
	...
	err := svc.SendMagicLink(context.Background(), testRecipient, int64(DefaultExpirySeconds), testFlowID, "")
	suite.NotNil(err)
	suite.Equal(ErrorTemplateRenderFailed.Code, err.Code)
}
...
func (suite *MagicLinkServiceTestSuite) TestSendMagicLinkNilEmailClient() {
	...
	err := svc.SendMagicLink(context.Background(), testRecipient, int64(DefaultExpirySeconds), testFlowID, "")
	suite.NotNil(err)
	suite.Equal(ErrorMagicLinkNotConfigured.Code, err.Code)
}
