...
type ConsentEnforcerServiceInterface interface {
	// ResolveConsent checks whether the user has provided required consents for the given application
	// and authorized permission set. Returns nil if all required consents are active; otherwise
	// returns ConsentPromptData describing which purposes/elements still need user consent.
...
// buildPurposePrompts dispatches each purpose to the per-type builder and returns the prompts that
// still require user consent. Purposes whose Type was not inferred (non-Thunder-owned) are skipped.
func buildPurposePrompts(purposes []consent.ConsentPurpose, essentialAttributes, optionalAttributes []string,
	...
	for _, purpose := range purposes {
		switch purpose.Type {
		case consent.PurposeTypeAttributes:
		...
		case consent.PurposeTypePermissions:
...
// buildAttributePurposePrompt builds a ConsentPurposePrompt for an attribute purpose, applying the
// same attribute, profile, and consented-element filtering as before.
func buildAttributePurposePrompt(purpose consent.ConsentPurpose,
	...
	return ConsentPurposePrompt{
		...
		Type:        string(consent.PurposeTypeAttributes),
		Essential:   essential,
		Optional:    optional,
	}, true
}
...
func buildPermissionPurposePrompt(purpose consent.ConsentPurpose,
	...
	return ConsentPurposePrompt{
		...
		PurposeID:   purpose.ID,
		Description: purpose.Description,
		Type:        string(consent.PurposeTypePermissions),
		Optional:    optional,
	}, true
}
...
func computePermissionParents(permissions []string) map[string]string {
	...
	for _, p := range permissions {
		...
		for _, q := range permissions {
			...
			next := rune(p[len(q)])
			if isPermissionDelimiterChar(next) && len(q) > len(longestParent) {
				longestParent = q
			}
		}
...
// isPermissionDelimiterChar reports whether r is a delimiter the resource service permits in
// permission strings (. _ : - /).
func isPermissionDelimiterChar(r rune) bool {
	switch r {
	case '.', '_', ':', '-', '/':
		return true
	}
