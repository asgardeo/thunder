...
func (suite *JWSUtilsTestSuite) TestJWKToPublicKeyComplexEC() {
	...
	publicKey, err := JWKToPublicKey(jwk)
	if err != nil {
		// If the EC point is not on the curve, this is a valid error case for this test
		// The error message from crypto/ecdh includes curve name
		...
		assert.True(suite.T(), ok)
		assert.Equal(suite.T(), validX, ecKey.X)
		assert.Equal(suite.T(), validY, ecKey.Y)
	}
}
