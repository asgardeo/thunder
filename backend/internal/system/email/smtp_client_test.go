...
func (suite *SMTPClientTestSuite) TestNewSMTPClient_EmptyFrom_Error() {
	...
	suite.Error(err)
	suite.Nil(client)
	suite.True(errors.Is(err, ErrInvalidSender))
}
...
func (suite *SMTPClientTestSuite) TestNewSMTPClient_EmptyHost_Error() {
	...
	suite.Error(err)
	suite.Nil(client)
	suite.True(errors.Is(err, ErrInvalidHost))
}
...
func (suite *SMTPClientTestSuite) TestNewSMTPClient_InvalidPort_Error() {
	...
	suite.Error(err)
	suite.Nil(client)
	suite.True(errors.Is(err, ErrInvalidPort))
...
func (suite *SMTPClientTestSuite) TestNewSMTPClient_EmptyCredentials_Error() {
	...
	suite.Error(err)
	suite.Nil(client)
	suite.True(errors.Is(err, ErrInvalidCredentials))
...
func (suite *SMTPClientTestSuite) TestSendEmail_PlainText_Success() {
	...
	config := suite.getValidSMTPConfig("127.0.0.1", addr.Port)
	client, err := newSMTPClient(config)
	suite.Require().NoError(err)
...
func (suite *SMTPClientTestSuite) TestSendEmail_ConnectionError() {
	...
	suite.Error(err)
	suite.True(errors.Is(err, ErrSMTPConnection))
}
...
func (suite *SMTPClientTestSuite) TestSendEmail_CRLFInjection_Error() {
	...
	suite.True(errors.Is(err, ErrInvalidSubject))
}

func (suite *SMTPClientTestSuite) TestBuildMessage_PlainText() {
	config := suite.getValidSMTPConfig("localhost", 25)
	ci, err := newSMTPClient(config)
	suite.Require().NoError(err)
...
func (suite *SMTPClientTestSuite) TestGetRecipients() {
	config := suite.getValidSMTPConfig("localhost", 25)
	ci, err := newSMTPClient(config)
	suite.Require().NoError(err)
	client := ci.(*smtpClient)
	...
	recipients := client.getRecipients(emailData)
...
func (suite *SMTPClientTestSuite) TestSendViaSMTP_AuthError() {
	...
	suite.Error(err)
	suite.True(errors.Is(err, ErrSMTPAuth))
	suite.waitForDone(done)
}
...
func (suite *SMTPClientTestSuite) TestSendViaSMTP_WriteError() {
	...
	errStr := err.Error()
	suite.True(errors.Is(err, ErrorEmailSendFailed) || errors.Is(err, ErrSMTPConnection),
		"Expected error to wrap ErrorEmailSendFailed or ErrSMTPConnection, but got: %s", errStr)
	suite.waitForDone(done)
}
