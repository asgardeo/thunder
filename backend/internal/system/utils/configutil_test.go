...
func (suite *ConfigUtilsTestSuite) TestSubstituteFilePaths_FilePathWithSpaces() {
	// The current regex pattern file://([^"\s]+) doesn't support file paths with spaces
	// because [^"\s]+ stops at whitespace characters. This test documents this limitation.
	...
	// Expected to fail because the regex only matches up to the first space
	// The function tries to read "file" instead of "file with spaces.txt"
	suite.Error(err, "File paths with spaces are not supported by the current regex pattern")
