...
func (suite *TransactionerTestSuite) TestTransactWithOptions_Success() {
	ctx := context.Background()
	opts := &sql.TxOptions{
	...
	// Expect transaction to be started with options and committed
	suite.mock.ExpectBegin()
	suite.mock.ExpectCommit()

	executed := false
	err := suite.transactioner.TransactWithOptions(ctx, opts, func(txCtx context.Context) error {
		executed = true
		suite.True(HasTx(txCtx))
		return nil
	})
...
func (suite *TransactionerTestSuite) TestTransactWithOptions_ExistingTransaction() {
	ctx := context.Background()
	...
	err := suite.transactioner.Transact(ctx, func(txCtx1 context.Context) error {
		...
		// Try to create a new transaction with different options
		// Should reuse existing transaction and ignore options
		opts := &sql.TxOptions{
			Isolation: sql.LevelSerializable,
			ReadOnly:  true,
		}

		err := suite.transactioner.TransactWithOptions(txCtx1, opts, func(txCtx2 context.Context) error {
			innerExecuted = true
			// Should reuse the same transaction
			suite.Equal(TxFromContext(txCtx1), TxFromContext(txCtx2))
			return nil
		})
