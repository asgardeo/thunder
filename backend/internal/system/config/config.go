...
func (c *ACRAMRMappingConfig) Validate() error {
	for amrKey := range c.AMR {
		if strings.TrimSpace(amrKey) == "" {
			return fmt.Errorf("acr_amr_mapping: AMR key must not be empty")
		}
	}
	...
	for acr, amrKeys := range c.AcrAMR {
		if strings.TrimSpace(acr) == "" {
			return fmt.Errorf("acr_amr_mapping: ACR value must not be empty")
		}
		if len(amrKeys) == 0 {
		...
		for _, amrKey := range amrKeys {
			if strings.TrimSpace(amrKey) == "" {
				return fmt.Errorf("acr_amr_mapping: ACR %q references an empty AMR key", acr)
			}
