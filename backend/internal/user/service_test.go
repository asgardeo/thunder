...
func TestUserService_CreateUser_PropagatesStoreError(t *testing.T) {
	...
	created, svcErr := service.CreateUser(user)
...
func (f *fakeTransactioner) TransactWithOptions(
	ctx context.Context, _ *sql.TxOptions, txFunc func(context.Context) error,
) error {
	return f.Transact(ctx, txFunc)
}
