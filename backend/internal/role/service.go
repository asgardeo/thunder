...
type RoleServiceInterface interface {
	...
	CreateRole(ctx context.Context,
		role RoleCreationDetail) (*RoleWithPermissionsAndAssignments, *serviceerror.ServiceError)
	GetRoleWithPermissions(ctx context.Context, id string) (*RoleWithPermissions, *serviceerror.ServiceError)
	UpdateRoleWithPermissions(ctx context.Context, id string,
		role RoleUpdateDetail) (*RoleWithPermissions, *serviceerror.ServiceError)
...
func (rs *roleService) CreateRole(
	...
	if err := rs.roleStore.CreateRole(ctx, id, role); err != nil {
...
func (rs *roleService) GetRoleWithPermissions(ctx context.Context,
	id string) (*RoleWithPermissions, *serviceerror.ServiceError) {
	logger := log.GetLogger().With(log.String(log.LoggerKeyComponentName, loggerComponentName))
...
func (rs *roleService) UpdateRoleWithPermissions(
	...
	if err := rs.roleStore.UpdateRole(ctx, id, role); err != nil {
...
func (rs *roleService) AddAssignments(ctx context.Context, id string,
	...
	if err := rs.roleStore.AddAssignments(ctx, id, assignments); err != nil {
...
func (rs *roleService) RemoveAssignments(ctx context.Context, id string,
	assignments []RoleAssignment) *serviceerror.ServiceError {
	logger := log.GetLogger().With(log.String(log.LoggerKeyComponentName, loggerComponentName))
	...
	if err := rs.roleStore.RemoveAssignments(ctx, id, assignments); err != nil {
...
func (rs *roleService) validateAssignmentIDs(ctx context.Context,
	assignments []RoleAssignment) *serviceerror.ServiceError {
	logger := log.GetLogger().With(log.String(log.LoggerKeyComponentName, loggerComponentName))
...
func (rs *roleService) getDisplayNameForAssignment(ctx context.Context, assignment *RoleAssignment) (string, error) {
	...
	case AssigneeTypeGroup:
		groupResp, err := rs.groupService.GetGroup(ctx, assignment.ID)
		if err != nil { // Changed svcErr to err for syntactic correctness
			return "", fmt.Errorf("failed to get group: %w", errors.New(err.Error)) // Changed svcErr to err
		}
...
func (rs *roleService) validatePermissions(ctx context.Context,
	permissions []ResourcePermissions) *serviceerror.ServiceError {
	logger := log.GetLogger().With(log.String(log.LoggerKeyComponentName, loggerComponentName))
