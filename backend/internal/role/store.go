...
type roleStoreInterface interface {
	...
	GetAuthorizedPermissions(ctx context.Context, userID string, groupIDs []string,
...
func (s *roleStore) CreateRole(ctx context.Context, id string, role RoleCreationDetail) error {
	...
	return s.executeInTransaction(dbClient, func(tx dbmodel.TxInterface) error {
		...
		if err := addPermissionsToRole(ctx, tx, id, role.Permissions, s.deploymentID); err != nil {
			return err
		}
		if err := addAssignmentsToRole(ctx, tx, id, role.Assignments, s.deploymentID); err != nil {
			return err
		}
		return nil
	})
...
func (s *roleStore) UpdateRole(ctx context.Context, id string, role RoleUpdateDetail) error {
	...
	return s.executeInTransaction(dbClient, func(tx dbmodel.TxInterface) error {
		...
		if err := updateRolePermissions(ctx, tx, id, role.Permissions, s.deploymentID); err != nil {
			return err
		}
		return nil
	})
...
func (s *roleStore) AddAssignments(ctx context.Context, id string, assignments []RoleAssignment) error {
	...
	return s.executeInTransaction(dbClient, func(tx dbmodel.TxInterface) error {
		return addAssignmentsToRole(ctx, tx, id, assignments, s.deploymentID)
	})
}
...
func (s *roleStore) RemoveAssignments(ctx context.Context, id string, assignments []RoleAssignment) error {
	...
	return s.executeInTransaction(dbClient, func(tx dbmodel.TxInterface) error {
		for _, assignment := range assignments {
			_, err := tx.ExecContext(ctx, queryDeleteRoleAssignmentsByIDs, id, assignment.Type, assignment.ID, s.deploymentID)
			if err != nil {
...
func (s *roleStore) getRolePermissions(ctx context.Context, dbClient provider.DBClientInterface,
	id string) ([]ResourcePermissions, error) {
...
func addPermissionsToRole(
	...
	for _, resPerm := range permissions {
		for _, permission := range resPerm.Permissions {
			_, err := tx.ExecContext(ctx, queryCreateRolePermission, id, resPerm.ResourceServerID, permission, deploymentID)
			if err != nil {
...
func addAssignmentsToRole(
	...
	for _, assignment := range assignments {
		_, err := tx.ExecContext(ctx, queryCreateRoleAssignment, id, assignment.Type, assignment.ID, deploymentID)
		if err != nil {
...
func updateRolePermissions(
	...
	_, err := tx.ExecContext(ctx, queryDeleteRolePermissions, id, deploymentID)
	if err != nil {
		return fmt.Errorf("failed to delete existing role permissions: %w", err)
	}
	err = addPermissionsToRole(ctx, tx, id, permissions, deploymentID)
	if err != nil {
...
func (s *roleStore) CheckRoleNameExistsExcludingID(ctx context.Context,
	ouID, name, excludeRoleID string) (bool, error) {
	dbClient, err := s.getIdentityDBClient()
	if err != nil {
		return false, err
	}
	results, err := dbClient.QueryContext(ctx, queryCheckRoleNameExistsExcludingID, ouID, name,
		excludeRoleID, s.deploymentID)
	if err != nil {
		return false, fmt.Errorf("failed to check role name existence: %w", err)
	}
	return parseBoolFromCount(results)
