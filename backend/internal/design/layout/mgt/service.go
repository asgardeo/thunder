...
func (ls *layoutMgtService) DeleteLayout(id string) *serviceerror.ServiceError {
	...
	if count > 0 {
		return &serviceerror.ServiceError{
			Code:             "LAY-1006",
			Error:            "Layout in use",
			ErrorDescription: fmt.Sprintf("Layout is being used by %d application(s)", count),
		}
	}
...
func validatePaginationParams(limit, offset int) *serviceerror.ServiceError {
	if limit < 1 || limit > serverconst.MaxPageSize {
		return &serviceerror.ServiceError{
			Code:             "LAY-1007",
			Error:            "Invalid limit",
			ErrorDescription: fmt.Sprintf("Limit must be between 1 and %d", serverconst.MaxPageSize),
		}
	}
