...
/**
 * Parameters for useGetLayouts hook
 */
export interface UseGetLayoutsParams {
  limit?: number;
  offset?: number;
}

/**
 * React Query hook to fetch layouts list
 *
 * @param params - Optional pagination parameters
 * @returns Query result containing layout list
 */
export default function useGetLayouts(params?: UseGetLayoutsParams): UseQueryResult<LayoutListResponse> {
  ...
  return useQuery<LayoutListResponse>({
    queryKey: [LayoutQueryKeys.LAYOUTS, {limit, offset}],
    queryFn: async (): Promise<LayoutListResponse> => {
      ...
      const response: {data: LayoutListResponse} = await http.request({
