...
// Note: We need to use a factory function to access t() for validation messages
...
export default function EditTokenSettings({
  ...
  // Fetch all user types and their schemas once on mount
  useEffect(() => {
    const fetchUserTypesAndSchemas = async () => {
      ...
      try {
        // First, get all user types
        const userTypesResponse = await http.request({
          url: `${API_BASE_URL}/user-schemas?limit=100`,
          method: 'GET',
        } as unknown as Parameters<typeof http.request>[0]);

        const userTypesData = userTypesResponse.data as {schemas: {id: string; name: string}[]};
        ...
        // Fetch full schemas for allowed user types
        const schemaPromises = allowedIds.map((id) =>
          http.request({
        ...
        const schemas = responses.map((response) => response.data as ApiUserSchema);
    ...
    fetchUserTypesAndSchemas().catch(() => {
      // Silently handle errors
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Extract and combine flattened user attributes from all allowed user schemas
  ...
  const isLoadingUserAttributes = schemasLoading;

  // Get user attributes based on mode
  ...
  useEffect(() => {
    if (pendingAdditions.size > 0) {
      const currentAttrs = isOAuthMode
