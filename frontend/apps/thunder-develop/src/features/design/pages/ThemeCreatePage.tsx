...
import {useCreateTheme, useGetTheme, useGetThemes} from '@thunder/shared-design';
...
export default function ThemeCreatePage(): JSX.Element {
  ...
  const baseThemeId = useMemo(() => {
    ...
    return themes.find((t) => t.displayName.toLowerCase() === 'classic')?.id ?? themes[0]?.id ?? null;
  }, [themesData]);
  ...
  const handleCreate = (): void => {
    ...
    createTheme.mutate(
      ...
      {
        onSuccess: (created) => {
          void navigate(`/design/themes/${created.id}`);
        },
        onError: (err: Error) => {
