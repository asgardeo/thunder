...
export default function RoleEditPage(): JSX.Element {
  ...
  const hasChanges = useMemo(
    () => Object.keys(editedRole).length > 0,
    [editedRole],
  );

  const effectiveDescription =
    'description' in editedRole ? (editedRole.description ?? '') : (role?.description ?? '');
  ...
  return (
    <PageContent>
              ...
              <TextField
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    const trimmed = tempName.trim();
                    const current = (editedRole.name ?? role.name).trim();
                    if (trimmed && trimmed !== current) handleFieldChange('name', trimmed);
                    setIsEditingName(false);
              ...
              <TextField
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && e.ctrlKey) {
                    const trimmed = tempDescription.trim();
                    if (trimmed !== effectiveDescription) handleFieldChange('description', trimmed || undefined);
                    setIsEditingDescription(false);
                  } else if (e.key === 'Escape') {
      ...
      {/* Tabs */}
      <Tabs value={activeTab} onChange={handleTabChange} aria-label="role settings tabs">
        <Tab label={t('roles:edit.page.tabs.general')} id="role-tab-0" aria-controls="role-tabpanel-0" sx={{textTransform: 'none'}} />
        <Tab label={t('roles:edit.page.tabs.assignments')} id="role-tab-1" aria-controls="role-tabpanel-1" sx={{textTransform: 'none'}} />
      </Tabs>
