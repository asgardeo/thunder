...
- `meta.components[].validation` - returned to the Login Gate and the Asgardeo JavaScript SDK so that clients can optionally pre-validate values before submission.
...
|---|---|---|
| `regex` | `string` | The submitted value must match the pattern. The server compiles the pattern with Go's RE2 engine, which guarantees linear-time matching and rejects Perl-only constructs such as lookarounds. |
