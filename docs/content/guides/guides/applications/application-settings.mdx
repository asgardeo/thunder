...
You can also configure the ID token response format:
...
**User Info** — configure what the `/userinfo` endpoint returns and in what format.
...
The **Advanced Settings** tab shows the OAuth 2.0 settings configured at creation. These are read-only in the Console.
...
| **Pushed Authorization Requests** | Whether the application must submit authorization parameters via the `/oauth2/par` endpoint before starting a flow (RFC 9126). |
