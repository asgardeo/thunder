...
:::info Auto-imports
The `@thunderid/nuxt` module auto-registers all `ThunderID*` components and auto-imports all composables. You do **not** need to import them manually.
:::

## Add Sign-In and Sign-Out

The module provides `<ThunderIDSignedIn>`, `<ThunderIDSignedOut>`, `<ThunderIDSignInButton>`, and `<ThunderIDSignOutButton>` components for conditional rendering and authentication actions.
...
<template>
  <main>
    <header>
      ...
      <ThunderIDSignedIn>
        <ThunderIDSignOutButton>Sign Out</ThunderIDSignOutButton>
      </ThunderIDSignedIn>
      <ThunderIDSignedOut>
...
Use `<ThunderIDUser>` with a scoped slot to access and display profile information for the signed-in user:
...
:::tip Protecting a page
To require authentication on a page, add the built-in `'auth'` route middleware:
