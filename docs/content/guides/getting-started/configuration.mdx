...
:::tip
The system resource server is a normal resource server that can be updated via the resource server APIs. When you update its handle, the permission strings in roles are cascaded automatically. After updating the handle, set `server.security.system_permission_prefix` to the new handle value and restart the server. You must also update the Console scopes to match the new permission strings.
:::
