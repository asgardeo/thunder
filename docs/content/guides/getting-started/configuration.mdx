...
| `database.runtime.hostname` | `""` | Database server hostname (PostgreSQL only) |
| `database.runtime.port` | `0` | Database server port (PostgreSQL only) |
| `database.runtime.name` | `""` | Database name (PostgreSQL only) |
| `database.runtime.username` | `""` | Database username (PostgreSQL only); Redis ACL username — leave empty if ACLs are not configured (Redis only) |
| `database.runtime.password` | `""` | Database password (PostgreSQL only); Redis password or ACL user password (Redis only) |
| `database.runtime.sslmode` | `""` | SSL mode for PostgreSQL (PostgreSQL only) |
| `database.runtime.path` | `repository/database/runtimedb.db` | SQLite database file path (SQLite only) |
| `database.runtime.options` | `_journal_mode=WAL&_busy_timeout=5000&_pragma=foreign_keys(1)` | Database-specific connection options (SQLite/PostgreSQL only) |
| `database.runtime.max_open_conns` | `500` | Maximum number of open connections (SQLite/PostgreSQL only) |
| `database.runtime.max_idle_conns` | `100` | Maximum number of idle connections (SQLite/PostgreSQL only) |
| `database.runtime.conn_max_lifetime` | `3600` | Maximum connection lifetime in seconds (SQLite/PostgreSQL only) |
| `database.runtime.address` | `""` | Redis server address in `host:port` format, for example `localhost:6379` (Redis only) |
| `database.runtime.db` | `0` | Redis logical database index (0–15) (Redis only) |
| `database.runtime.key_prefix` | `""` | Prefix applied to all Redis keys written by Thunder, for example `thunder:` (Redis only) |
...
- Thunder does not enforce TLS at the client configuration level. To encrypt traffic, place a TLS-terminating proxy in front of Redis and point `database.runtime.address` at the proxy endpoint.
- If your Redis deployment uses Access Control Lists (ACLs), create a dedicated user and grant the following commands: `GET`, `SET`, `DEL`, `EXPIRE`, `EVAL`, `EVALSHA`. Set `database.runtime.username` and `database.runtime.password` accordingly.
